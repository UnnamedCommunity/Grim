...
public class PredictionEngineElytra extends PredictionEngine {
    ...
    public List<VectorData> applyInputsToVelocityPossibilities(GrimPlayer player, Set<VectorData> possibleVectors, float speed) {
        ...
        for (VectorData data : possibleVectors) {
            Vector elytraResult = getElytraMovement(player, data.vector.copy(), currentLook).mul(player.stuckSpeedMultiplier).mul(0.99F, 0.98F, 0.99F);
            ...
            elytraResult = getElytraMovement(player, data.vector.copy(), ReachUtils.getLook(player, player.xRot, player.yRot)).mul(player.stuckSpeedMultiplier).mul(0.99F, 0.98F, 0.99F);
            player.trigHandler.toggleShitMath();
            results.add(data.returnNewModified(elytraResult, VectorData.VectorType.InputResult));
        }
    ...
    public static Vector getElytraMovement(GrimPlayer player, Vector vector, Vector lookVector) {
        ...
        double horizontalLength = vector.copy().setY(0).length();
        ...
        vector.addY(recalculatedGravity * (-1.0D + vertCosRotation * 0.75D));
        double d5;
        ...
        if (horizontalSqrt > 0) {
            vector.addXZ((lookVector.getX() / horizontalSqrt * horizontalLength - vector.getX()) * 0.1D, (lookVector.getZ() / horizontalSqrt * horizontalLength - vector.getZ()) * 0.1D);
        }
